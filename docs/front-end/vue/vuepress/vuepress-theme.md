...
默认主题提供了一个主页布局（用于[该网站的主页](/README.md)）。要使用它，需要在你的根目录 `README.md` 的 [YAML front matter](./vuepress-guide.md#front-matter) 中指定 `home：true`，并加上一些其他的元数据。这是本网站使用的实际数据：
...
导航栏包括页面标题、[搜索框](#search-box), [导航栏链接](#navbar-links)、[多语言支持](#) 和 [仓库链接](#git-repo-and-edit-links)，它们都取决于你的配置。
...
::: tip 提示
请不要将 `themeConfig.serviceWorker` 与 [Config > serviceWorker](#) 混淆，[Config > serviceWorker](#) 是**站点级别的**，而 `themeConfig.serviceWorker` 是**主题级别的**
:::
